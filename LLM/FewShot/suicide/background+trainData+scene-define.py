...
BATCH_SIZE = 20
prompt_intro = ["高自杀风险的判断标准为：（1）自杀可能正在进行中。（2）自杀方式已确定，近日内可能进行。（3）自杀已经计划中，自杀日期大体确定。（4）自杀方式已确定，自杀日期未明。（5）自杀已经计划中，自杀日期未明。\n低自杀风险的判断标准为：（1）强烈的自杀愿望，自杀方式未明。（2）自杀愿望已表达，具体方式和计划未明。（3）强烈的生存痛苦，未见自杀愿望表达。（4）生存痛苦已明确表达，未见自杀愿望表达。（5）生存病苦有所表达，未见自杀愿望表达。（6）未见任何生存痛苦表达。\n下方内容分别显示了用户的ID和其在社交媒体上的发文内容。请你结合上述高自杀风险和低自杀风险的判断标准，进行二分类任务，判断以下帖子是低自杀风险还是高自杀风险。请你用0、1标注高低自杀风险，0表示低自杀风险，1表示高自杀风险，请你用markdown表格的形式输出，输出结果的格式如下：表头依次为id、标签。注意：请只生成MarkDown表格，其他文字不要有。接下来我将给你几个帖子以及其相对应的专家标注结果（注意：以下标签0代表低自杀风险，1代表高自杀风险），请你学习下面的例子。\n"]
