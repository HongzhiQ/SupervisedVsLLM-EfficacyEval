...
BATCH_SIZE =25
prompt_intro = ["下方内容分别显示了用户的ID和其在社交媒体上的发文内容。你是心理学专家，请你考虑下述帖子中表达的心理健康状况，并进行二分类任务，判断以下帖子是低自杀风险还是高自杀风险。请你用0、1标注高低自杀风险，0表示低自杀风险，1表示高自杀风险，请你用markdown表格的形式输出，输出结果的格式如下：表头依次为id、标签。注意：请只生成MarkDown表格，其他文字不要有。接下来我将给你几个帖子以及其相对应的专家标注结果（注意：以下标签0代表低自杀风险，1代表高自杀风险），请你学习下面的例子。\n"]
